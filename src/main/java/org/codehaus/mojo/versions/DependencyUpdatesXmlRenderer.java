...
public class DependencyUpdatesXmlRenderer {
    ...
    public DependencyUpdatesXmlRenderer(Map<Dependency, ArtifactVersions> dependencyUpdates, Map<Dependency, ArtifactVersions> dependencyManagementUpdates,
    ...
    public void render() throws MavenReportException {
        ...
        sb.append(getDependencyInfoBlock(dependencyManagementUpdates, "dependencyManagements", "dependencyManagement"));
    ...
    /**
     * Returns summary of dependency analysis result in xml format: current version, next available, next incremental, next
     * minor and next major versions.
     *
    ...
    public static String getSummaryBlock(Collection<ArtifactVersions> allUpdates) {
        ...
        String result = "\t<summary>%n" + "\t\t<usingLastVersion>%d</usingLastVersion>%n" + "\t\t<nextVersionAvailable>%d</nextVersionAvailable>%n"
                + "\t\t<nextIncremetalAvailable>%d</nextIncremetalAvailable>%n" + "\t\t<nextMinorAvailable>%d</nextMinorAvailable>%n"
                + "\t\t<nextMajorAvailable>%d</nextMajorAvailable>%n" + "\t</summary>%n";

        return String.format(result, numCur, numAny, numInc, numMin, numMaj);
    }

    /**
     * Returns xml report for current dependency state with following info: current version, next available version, next
     * incremental/minor/major if available and status ('incremental available', 'minor available', 'major available' or 'no
     * new available')
     *
    ...
    public static String getVersionsBlocks(ArtifactVersions versions) {
        ...
        sBuilder.append(TAB).append(TAB).append(TAB).append(wrapElement(versions.getCurrentVersion().toString(), CURRENT_VERSION)).append(NL);
        ArtifactVersion nextVersion = versions.getOldestUpdate(UpdateScope.ANY);
        if (nextVersion != null) {
            sBuilder.append(TAB).append(TAB).append(TAB).append(wrapElement(nextVersion.toString(), NEXT_VERSION)).append(NL);
            ...
            sBuilder.append(TAB).append(TAB).append(TAB).append(wrapElement("no new available", STATUS)).append(NL);
        }
    ...
    private static String getDependencyInfoBlock(Map<Dependency, ArtifactVersions> dependencyUpdates, String blockName, String subblockName) {
        ...
        for (Entry<Dependency, ArtifactVersions> entry : dependencyUpdates.entrySet()) {
            ...
            sBuilder.append(TAB).append(TAB).append(TAB).append(wrapElement(dep.getGroupId(), GROUP_ID)).append(NL);
            sBuilder.append(TAB).append(TAB).append(TAB).append(wrapElement(dep.getArtifactId(), ARTIFACT_ID)).append(NL);
            sBuilder.append(TAB).append(TAB).append(TAB).append(wrapElement(dep.getScope(), SCOPE)).append(NL);
            sBuilder.append(TAB).append(TAB).append(TAB).append(wrapElement(dep.getClassifier(), CLASSIFIER)).append(NL);
            sBuilder.append(TAB).append(TAB).append(TAB).append(wrapElement(dep.getType(), TYPE)).append(NL);
    ...
    private static String getVersionsInScopeBlock(ArtifactVersions av, UpdateScope scope) {
        ...
        if (nextVersion != null) {
            ...
            for (ArtifactVersion version : versions) {
                sBuilder.append(TAB).append(TAB).append(TAB).append(TAB).append(wrapElement(version.toString(), scope.toString().toLowerCase())).append(NL);
            }
