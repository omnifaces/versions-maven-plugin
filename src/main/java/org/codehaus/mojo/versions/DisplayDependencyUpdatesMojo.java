...
public class DisplayDependencyUpdatesMojo extends AbstractVersionsDisplayMojo {
    ...
    /**
     * Whether to allow the major version number to be changed. You need to set {@link #allowAnyUpdates} to
     * <code>false</code> to get this configuration gets control.
     *
     * @since 2.5
     */
    ...
    /**
     * Whether to allow the minor version number to be changed. You need to set {@link #allowMajorUpdates} to
     * <code>false</code> to get this configuration gets control.
     *
     * @since 2.5
     */
    ...
    /**
     * Whether to allow the incremental version number to be changed. You need to set {@link #allowMinorUpdates} to
     * <code>false</code> to get this configuration gets control.
     *
     * @since 2.5
     */
    ...
    /**
     * Whether to allow any version change to be allowed. This keeps compatibility with previous versions of the plugin. If
     * you set this to false you can control changes in version number by {@link #allowMajorUpdates},
     * {@link #allowMinorUpdates} or {@link #allowIncrementalUpdates}.
     *
        ...
        for (Iterator<Dependency> i = dependencies.iterator(); i.hasNext();) {
            ...
            while (!matched && j.hasNext()) {
                ...
                if (StringUtils.equals(t.getGroupId(), c.getGroupId()) && StringUtils.equals(t.getArtifactId(), c.getArtifactId())
                        ...
                        && (c.getVersion() == null || t.getVersion() == null || StringUtils.equals(t.getVersion(), c.getVersion()))) {
                    matched = true;
                    break;
                }
            }
    ...
    public void execute() throws MojoExecutionException, MojoFailureException {
                ...
                if (dependency.getVersion() == null) {
                    ...
                    if (getProject().hasParent()) {
                        ...
                        if (getProject().getParent().getDependencyManagement() != null) {
                            List<Dependency> parentDeps = getProject().getParent().getDependencyManagement().getDependencies();
                            for (Dependency parentDep : parentDeps) {
                                // only groupId && artifactId needed cause version is null
                                if (dependency.getGroupId().equals(parentDep.getGroupId()) && dependency.getArtifactId().equals(parentDep.getArtifactId())
                        ...
                        String message = "We can't get the version for the dependency " + dependency.getGroupId() + ":" + dependency.getArtifactId()
                                + " cause there does not exist a parent.";
        ...
        if (isProcessPluginDependenciesInDependencyManagement()) {
            pluginDependenciesInPluginManagement = extractPluginDependenciesFromPluginsInPluginManagement(getProject().getBuild());
        }
        ...
        try {
            ...
            if (isProcessPluginDependenciesInDependencyManagement()) {
                logUpdates(getHelper().lookupDependenciesUpdates(pluginDependenciesInPluginManagement, false), "pluginManagement of plugins");
            }
            if (isProcessingPluginDependencies()) {
    ...
    private void logUpdates(Map<Dependency, ArtifactVersions> updates, String section) {
        ...
        while (i.hasNext()) {
            ...
            if (versions.isCurrentVersionDefined()) {
                ...
                ArtifactVersion newestVersion = versions.getNewestVersion(versions.getArtifact().getVersionRange(), allowSnapshots);
                current = versions.getArtifact().getVersionRange().toString();
                latest = newestVersion == null ? null : versions.getNewestUpdate(newestVersion, calculateUpdateScope(), allowSnapshots);
